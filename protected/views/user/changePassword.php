...
function check()
{
    var pass=$('#ChangePassword_password').val();
    var newPass=$('#ChangePassword_newPassword').val();
    var conPass=$('#ChangePassword_conformPassword').val();
    var currPass=$('#currentPass').val();
    
    
    if(md5(pass)==currPass)
        {
            if(newPass!='' && conPass!='')
                {
                    if(newPass==conPass)
...
<input type="hidden" id="currentPass" value="<?php echo $password;?>" />
<a href='javascript:void(0)'><?php $this->widget('bootstrap.widgets.BootButton', array('buttonType'=>'submit', 'icon'=>'ok', 'label'=>'Submit')); ?></a>
