 ...
 *
 */
class EImageColumn_businessPages extends CGridColumn
{
	public $name;
	...
	{
            //print_r($data['attributes']);return;
            
               if($data->image!='' && $data->image!='user.png')
               {
                   $image = CHtml::image($this->pathPrefix .'business_page_'.$data->user_id.'/'.$data->image . $this->pathSuffix,
				$this->alt,
				$this->htmlOptions);
               ...
               {
                   $image = CHtml::image($this->pathPrefix .$data->image . $this->pathSuffix,
				$this->alt,
				$this->htmlOptions);
