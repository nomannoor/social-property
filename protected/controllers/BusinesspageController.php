...
class BusinesspageController extends Controller {
    ...
    public function actionViewpage($id){
     ...
     public function actionupdatePost($message,$id) {
        ...
        $page_creater_name = pages::model()->findByPk($id);
        $name = UserProfile::model()->find('user_id=:uid',array(':uid'=>Yii::app()->user->userId));
        $text = $name['first_name']. " " .$name['last_name']." Posted on your business page <b>".$message."</b>";
        $date = date('Y-m-d H:i:s');
        $this->notification(Yii::app()->user->userId,$page_creater_name->user_id,$text,$date,'page_post');
    ...
    public function actionpostlike($postId) {
        ...
        $model->date = date('Y-m-d H:i:s');
        $model->save();
        $page_creater_name = BusinessPagePost::model()->findByPk($postId);
        $page = Pages::model()->findByPk($page_creater_name->page_id);
        $name = UserProfile::model()->find('user_id=:uid',array(':uid'=>Yii::app()->user->userId));
        $text = $name['first_name']. " " .$name['last_name']." Like on your business page Post <b>".$page_creater_name->message."</b>";
        $date = date('Y-m-d H:i:s');
        $this->notification($page->user_id,Yii::app()->user->userId,$text,$date,'page_post');
...
//                    }
    ...
    public function actionAllbusinesspages()
    {
        
        $this->layout = 'layout_user';
        
        $this->render('allbusinesspages');
    }
