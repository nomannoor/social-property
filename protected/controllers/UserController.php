     ...
     if($size<(1024*1024*1024))
      {
       ...
       $tmp = $_FILES['photoimg']['tmp_name'];
                                                                   ...
                                                                   $table.='class="well-for-table">';
                                                                    ...
                                                                    $table .='<tr style="display:none;" id="comment_t'.$Newpost['id'].'"><td>&nbsp;</td><td>&nbsp;&nbsp;&nbsp;</td><td><textarea id="commenttext_'.$Newpost['id'].'" style="width:98%; height:40px; resize: none;" placeholder="Enter your comment here..."></textarea><input style="float:right;margin-right:5px;" type="button" value="Comment" onclick="addrecentComment('.$Newpost['id'].')"></td></tr>';
                                                                    echo $table;
    ...
    public function actionupdatePost() {
        ...
        if(!empty($tag))
        {
            ...
            foreach($tagging as $t):
               ...
               if($i!=0){
               $name = UserProfile::model()->find('user_id=:uid',array(':uid'=>Yii::app()->user->userId));
               $text = $name['first_name']. " " .$name['last_name']." Tag on your wall <b>".$message."</b>";
               $date = date('Y-m-d H:i:s');
               $this->notification(Yii::app()->user->userId,$t,$text,$date,'taggin');
               
               }
               $i++;
            endforeach;
            $noti_senderId = $tagging[0];
            $noti_userId = $tagging[1];
        /*$name = UserProfile::model()->find('user_id=:uid',array(':uid'=>$noti_senderId));
        $text = $name['first_name']. " " .$name['last_name']." Wrote on your wall <b>".$message."</b>";
        $date = date('Y-m-d H:i:s');
        $this->notification($noti_senderId,$noti_userId,$text,$date,'wall_post');
          */  
        }
